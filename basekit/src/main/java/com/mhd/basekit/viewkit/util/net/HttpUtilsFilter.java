...
import com.example.muheda.mhdsystemkit.sytemUtil.functionutil.SPUtil;
...
import com.example.muheda.mhdsystemkit.sytemUtil.functionutil.MD5Util;
...
//        url = ceshi(url);
    ...
    private static boolean check(String url) {
        ...
        /*(url.contains(Common.url)
                || url.contains(Common.mallurl) || url.contains(Common.carUrl5))*/
        return !TextUtils.isEmpty(url) && isContainsUrls(url);
    }
